// Placeholder catalogue for /collections and /products/:slug — see COLLECTIONS_DESIGN_PLAN.md §2.2.
// Structurally correct, numerically fictional: every price/MOQ figure needs a real-data pass
// before ship, same flag already used for the Pricing and Heritage tables. Prices are drawn
// from Pricing page's own placeholder tiers where the garment matches (tee/hoodie), so the two
// pages don't quietly contradict each other.
//
// `garmentModel` is only set where the Atelier's 3D viewer actually has a matching mesh
// (t-shirt.glb / hoodie.glb, see GarmentViewer.jsx) — omitted for silhouettes it can't render,
// so the product page never offers a 3D toggle that would show the wrong garment.
//
// Colour hexes are drawn from the Atelier's own COLOURS palette (AtelierPage.jsx) rather than
// invented per product, so every "Customize in the Atelier" deep link resolves to a real,
// selectable colour instead of snapping to the nearest one.
//
// No per-product photography exists yet, so every product borrows its category's existing
// Lookbook/homepage photo as interim art (COLLECTIONS_DESIGN_PLAN.md §7) via `categoryImage()`
// below rather than pointing at image files that don't exist.
...
export function categoryImage(category) {
  return `/landing%20page%20images/${CATEGORY_IMAGE[category]}`
}
...
export const products = [
  ...
  },
  {
    ...
    // No `sizes` — footwear runs on numeric sizing, not the S–XL apparel scale used elsewhere;
    // an exact size-run gets worked out once a quote conversation starts (COLLECTIONS_DESIGN_PLAN.md §8.5).
    description:
